...
def crear_variable_endpoint(variable_data: CrearVariable, db: Session = Depends(get_db)):
    db_variable = get_variable_por_nombre(db, nombre=variable_data.nombre)
    if db_variable:
...
@router.delete("/eliminar_variable/{nombre}", response_model=RespuestaVariable)
def eliminar_variable(nombre: str, db: Session = Depends(get_db)):
...
@router.delete("/eliminar_variable/{numero}", response_model=RespuestaVariable)
def eliminar_variable(numero: int, db: Session = Depends(get_db)):
