...
const GraficoMedidor = ({ title, url, nodeId }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const finalUrl =
          nodeId !== undefined ? `${url}?node_id=${nodeId}` : url;
            ...
            datasets: [
              {
                label: `${title}`,
                data: [
                  lastPressureData.data,
                  1050 - lastPressureData.data,
                  lastPressureData.data - 950,
                ], // Simula un medidor entre 950 y 1050 hPa
                backgroundColor: [
                  colorMode === "light"
                    ? "rgba(75, 192, 192, 1)"
                    : "rgba(54, 162, 235, 1)", // Color de la presión actual
                ...
                borderColor: [
                  colorMode === "light"
                    ? "rgba(75, 192, 192, 1)"
                    : "rgba(54, 162, 235, 1)", // Borde del medidor actual
        ...
        console.error(
          `Error al obtener los datos del resumen de ${title}:`,
          error
        );
      }
    };
      ...
      >
        ...
        {chartData ? (
          <Box height={{ base: "100px", md: "150px" }}>
            ...
            {/* Gráfico tipo Doughnut para el medidor */}
          </Box>
        ) : (
          <Text fontSize={{ base: "sm", md: "md" }}>Cargando gráfico...</Text>
        )}
      </Box>
